...
export default function DentistAppointments() {
  ...
  const fetchAppointments = useCallback(async (token: string, search: string, status: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Map backend MongoDB data to our Frontend UI format
        const formattedData: Appointment[] = data.map((apt: any, index: number) => {
          // Format date nicely (e.g., "April 12, 2026")
          ...
          return {
            ...
            avatarColor: getColorByIndex(index) // Assign dynamic color
          };
        });
  ...
  }, [router, fetchAppointments, activeFilter]); // Refetch if activeFilter changes
  ...
  const handleCancelAppointment = async (appointmentId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        // Optimistically update the UI without reloading the whole page
  ...
  const handleReschedule = (appointmentId: string) => {
    ...
    alert(`Reschedule flow for appointment ${appointmentId} will open a calendar modal here!`);
  };
  ...
  const getStatusBadge = (status: AppointmentStatus) => {
    switch (status) {
      case 'Confirmed':
        return (
          <span className="flex items-center gap-1.5 px-3 py-1 bg-blue-100 text-blue-600 rounded-full text-xs font-bold">
            <CheckCircle2 className="w-3.5 h-3.5" /> Confirmed
          </span>
        );
      case 'In Progress':
        return (
          <span className="flex items-center gap-1.5 px-3 py-1 bg-blue-50 text-blue-500 rounded-full text-xs font-bold border border-blue-100">
            <Clock4 className="w-3.5 h-3.5" /> In Progress
          </span>
        );
      case 'Pending':
        return (
          <span className="flex items-center gap-1.5 px-3 py-1 bg-orange-50 text-orange-500 rounded-full text-xs font-bold border border-orange-100">
            <AlertCircle className="w-3.5 h-3.5" /> Pending
          </span>
        );
      case 'Cancelled':
        return (
          <span className="flex items-center gap-1.5 px-3 py-1 bg-red-50 text-red-500 rounded-full text-xs font-bold border border-red-100">
      ...
      <aside className="w-[260px] bg-white border-r border-gray-200 flex flex-col fixed h-full z-10">
        <div className="h-20 flex items-center px-8 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full border border-blue-500 flex items-center justify-center text-blue-600 font-bold text-xs">D+</div>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              ...
              <motion.button 
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => alert("Open New Appointment Form Modal")}
